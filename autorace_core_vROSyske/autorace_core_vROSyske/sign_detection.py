...
class Sign_detection(Node):
    ...
    def __init__(self):
        ...
        # Имя файла для записи видео
        self.video_path = os.path.join(self.save_dir, 'output.mp4')

        # Параметры видео
        self.frame_width = 640  # Ширина кадра (можно изменить)
        self.frame_height = 480  # Высота кадра (можно изменить)
        self.fps = 30  # FPS (кадры в секунду)

        # Создаем объект VideoWriter
        self.video_writer = cv2.VideoWriter(
            self.video_path,
            cv2.VideoWriter_fourcc(*'mp4v'),  # Кодек для записи видео
            self.fps,
    ...
    def subscription_callback(self, image_msg):
        ...
        # Преобразуем изображение обратно в BGR для записи видео
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    ...
    def __del__(self):
        # Закрываем VideoWriter при завершении работы узла
